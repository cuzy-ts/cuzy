...
import type { Connection } from "../../../../servers/fizz/connection";
import type { IHttpRequest } from "../../../../servers/fizz/http/request";
...
export class PusherController {
  ...
  /**
   * Invoke the Fizz WebSocket server.
   *
  ...
  __invoke(
    ...
    const fizzConnection = this.connection(request, connection, appKey);

    if (!fizzConnection) {
      return;
    }

    connection.withMaxMessageSize(fizzConnection.app().maxMessageSize());

    connection.onMessage((message: string | Buffer) =>
      this.server.message(fizzConnection, String(message)),
    );

    connection.onControl((message: Frame) =>
      this.server.control(fizzConnection, message),
    );

    connection.onClose(() => this.server.close(fizzConnection));

    // Note: openBuffer() is not needed in TypeScript implementation
    // as Bun's WebSocket handles buffering automatically

    this.server.open(fizzConnection);
  }

  /**
   * Get the Fizz connection instance for the request.
   *
   * Looks up the application by key and creates a new FizzConnection
   ...
   * @returns The Fizz connection instance, or null if the application is invalid
   */
