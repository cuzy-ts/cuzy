/**
 * Factory for creating Fizz WebSocket servers
 *
...
import type { FizzConfig, FizzServerConfig } from "../../config/types";
import { Connection as FizzConnection } from "../../connection";
...
import { FizzServerProvider } from "./fizz-server-provider";
...
/**
 * Options for creating a Fizz server
 */
...
/**
 * Result of creating a Fizz server
 */
...
/**
 * Factory for creating Fizz WebSocket servers with Pusher protocol support
 *
 * @class Factory
 * @example
 * ```typescript
...
export class Factory {
   ...
   *
   * @param config - The Fizz configuration
   */
  public static initialize(config: FizzConfig): void {
    ...
    // Create FizzServerProvider early so we can pass it to ClientEvent
    const fizzServerProvider = new FizzServerProvider();
    Factory.serverProvider = fizzServerProvider;
    ...
    if (serverConfig?.scaling?.enabled) {
      ...
      Factory.pubSubProvider = new RedisPubSubProvider(
        Factory.logger,
        incomingHandler,
        serverConfig.scaling.channel || "fizz",
        redisConfig,
      );

      // Wire up the cycle
      fizzServerProvider.setPubSubProvider(Factory.pubSubProvider);
      ...
      websocket: {
        open: (ws: ServerWebSocket<unknown>) => {
          ...
          try {
            ...
            // Create Fizz connection with app and origin
            const connection = new FizzConnection(
              wsConnection,
              data.app,
              data.origin || null,
            );
...
/**
 * Create and start a Fizz WebSocket server
 *
