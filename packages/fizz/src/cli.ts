...
/**
 * Fizz CLI - Command-line interface for managing the WebSocket server
 *
 * A TypeScript port of Laravel Fizz's CLI commands
 *
 * @module cli
 */

import { createServer } from "./servers/fizz/factory";
...
function displayHelp(): void {
  console.log(`
Fizz - Bun-powered WebSocket server implementing the Pusher protocol
...
EXAMPLES:
  fizz start                                    # Start with default config
  fizz start --host 127.0.0.1 --port 8080       # Start with custom host/port
  fizz start --debug                            # Start with debug logging
  fizz start --config ./fizz.config.ts        # Start with custom config

CONFIGURATION:
  Fizz looks for configuration in the following locations:
...
function displayVersion(): void {
  ...
  console.log(`Fizz v${pkg.version}`);
...
async function startServer(
  ...
  console.log("🚀 Starting Fizz WebSocket Server");
  console.log("");
