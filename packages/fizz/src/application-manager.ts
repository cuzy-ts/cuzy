...
import type { FizzAppConfig, FizzConfig } from "./config/types";
...
export class ApplicationManager {
   ...
   */
  constructor(private readonly config: FizzConfig) {}
  ...
  public createConfigDriver(): ConfigApplicationProvider {
    const fizzApps: FizzAppConfig[] = this.config.apps?.apps ?? [];

    // Map FizzAppConfig to the format expected by ConfigApplicationProvider
    const apps = fizzApps.map((app) => {
      const config: {
        app_id: string;
        key: string;
        secret: string;
