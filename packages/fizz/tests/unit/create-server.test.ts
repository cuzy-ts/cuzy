...
import type { FizzConfig } from "@/config/types";
import { createServer, Factory } from "@/servers/fizz/factory";

describe("createServer", () => {
  const baseConfig: FizzConfig = {
    default: "fizz",
    servers: {
      fizz: {
        host: "127.0.0.1",
  ...
  describe("Config resolution", () => {
    it("should accept config object directly", async () => {
      ...
      const fizzConfig = baseConfig.servers.fizz;
      if (!fizzConfig) throw new Error("Fizz config undefined");
      expect(result.serverConfig).toBe(fizzConfig);

      await result.shutdown();
    });

    it("should load config from path if provided", async () => {
      const fixtureConfig: FizzConfig = {
        default: "test",
        servers: {
          test: {
            host: "127.0.0.1",
    ...
    it("should fallback to loadConfig when no config provided", async () => {
      Bun.env.FIZZ_APP_KEY = "env-key";
      Bun.env.FIZZ_APP_SECRET = "env-secret";
      Bun.env.FIZZ_APP_ID = "env-app";
      Bun.env.FIZZ_SERVER_PORT = "0";
      ...
      delete Bun.env.FIZZ_APP_KEY;
      delete Bun.env.FIZZ_APP_SECRET;
      delete Bun.env.FIZZ_APP_ID;
      delete Bun.env.FIZZ_SERVER_PORT;
    });
  ...
  describe("Server name selection", () => {
    ...
    it("should use first available server if default not found", async () => {
      const configWithoutDefault: FizzConfig = {
        default: "nonexistent",
        servers: {
          first: {
            host: "127.0.0.1",
  ...
  describe("Error handling", () => {
    ...
    it("should handle missing apps config", async () => {
      const configWithoutApps: FizzConfig = {
        default: "fizz",
        servers: {
          fizz: {
            host: "127.0.0.1",
  ...
  describe("Integration scenarios", () => {
    ...
    it("should support dynamic configuration", async () => {
      const dynamicConfig: FizzConfig = {
        default: "dynamic",
        servers: {
          dynamic: {
            host: "127.0.0.1",
