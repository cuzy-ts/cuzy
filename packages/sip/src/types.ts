...
// biome-ignore lint/suspicious/noExplicitAny: Compatibility - matches upstream
...
export type Broadcaster = {
  fizz: {
    ...
    options: GenericOptions<"fizz"> &
      Partial<CustomOmit<PusherOptions<"fizz">, "cluster">>;
  };
  pusher: {
    connector: PusherConnector;
    public: PusherChannel;
...
/**
 * Sip options for a specific broadcaster type.
 * This type is derived from the Broadcaster mapped type.
 */
export type SipOptions<TBroadcaster extends keyof Broadcaster> =
  Broadcaster[TBroadcaster]["options"];

/**
 * Sip options with all defaults applied.
 */
export type SipOptionsWithDefaults<
...
/**
 * Pusher-specific options that extend SipOptionsWithDefaults.
 */
export type PusherOptions<TBroadcastDriver extends BroadcastDriver> =
  SipOptionsWithDefaults<TBroadcastDriver> & {
    key: string;
