...
- **HTTP server & routing:** Bun-based HTTP/WebSocket server (`servers/fizz/http/server.ts`) with router, request/response abstractions, and CLI entrypoint `src/cli.ts` (`fizz start`) that calls `createServer`.
...
**Partially Ported / Not Yet Wired (Fizz scaling + metrics)**
These are the biggest gaps vs upstream Laravel Fizz:

- **Redis pub/sub scaling (multi-node):**
  - Redis pub/sub plumbing exists: `servers/fizz/publishing/redis-*.ts`, `IPubSubProvider` + `RedisPubSubProvider`, and `PusherPubSubIncomingMessageHandler`.
  - `config/types.ts` and `config/load.ts` define `ScalingConfig` and read `FIZZ_SCALING_ENABLED`, `FIZZ_SCALING_CHANNEL`, and Redis host/port/credentials.
...
- **Event dispatch via pub/sub:**
  ...
  - Result: message fan-out across multiple Fizz instances (via Redis) is not yet implemented, even though the building blocks exist.
...
- **Scaling config usage:**
  - `FizzServerConfig.scaling` and related env variables are read, but nothing in `Factory` or `createServer()` examines `scaling.enabled` or uses the Redis config to start pub/sub.
  - In PHP Fizz, `FizzServerProvider` uses the scaling config to register a Redis pub/sub provider and decide whether to publish events; in the TS port this role is currently reduced to a minimal `ServerProvider` that never publishes or subscribes.
...
- **Laravel service providers & DI glue:** `FizzServiceProvider`, `ApplicationManagerServiceProvider`, `ServerProviderManager`, etc. The TS port instead uses plain factories and config structs.
...
**Sip client vs Laravel Sip**

Core Sip package (`packages/sip`) is currently a subset/wrapper around upstream `laravel-sip`:

- **Connectors / broadcasters:**
  - Upstream `laravel-sip` supports `fizz`, `pusher`, `ably`, `socket.io`, `null`, and custom constructor-based broadcasters.
  - The local `Sip` class in `packages/sip/src/sip.ts` supports only `"fizz"` and `"pusher"` and always uses `PusherConnector`. There is no implementation of socket.io, ably, or null broadcasters in this class.
  - However, `packages/sip` depends on `laravel-sip`, and the React helpers in `packages/sip/src/react/config/index.ts` construct an upstream `Sip` instance from `"laravel-sip"`, not from the local `Sip` class. For React consumers, feature parity is effectively inherited from the upstream package.

- **Interceptors and `X-Socket-Id` behaviour:**
  - Laravel Sip’s `sip.ts` includes Axios/XMLHttpRequest/Turbo interceptors to attach `X-Socket-Id` headers automatically.
  - The local `Sip` class drops all of that; there are no built-in HTTP interceptors in `src/sip.ts`.
  - The React helpers also don’t set up interceptors; they only ensure `Pusher` is wired into the Sip config.
...
- **Extra channels:**
  ...
  - `packages/sip/src/channel` implements only the Pusher and null variants and omits socket.io-related channels and the null encrypted private channel.
  - Again, consumers using upstream `laravel-sip` via the React integration still get these features from that package, not from the local channel implementations.

**Overall status**

- For the **Fizz server**, all core single-node WebSocket/Pusher features are ported: applications, connections, channels, HTTP API, jobs, and the Bun-based HTTP/WebSocket server with CLI.
- The **main missing behaviours vs Laravel Fizz** are:
...
- For **Sip**, the local `Sip` class exposes only a subset of `laravel-sip` functionality and omits interceptors, but the React helpers intentionally rely on the upstream `laravel-sip` package, so React usage is effectively at parity with upstream Sip.
...
- **Bun Strategy**:
  ...
  - Port the Dashboard UI (React/Vue) to consume a JSON API served by Fizz.
...
- **Bun Strategy**:
  - Re-implement as a Bun CLI (`fizz sail`).
...
- **First Step**: Create a `sail` command in the Fizz CLI that checks for `docker` presence and writes a default `docker-compose.yml`.
...
- **Bun Strategy**:
  ...
  - Provide a minimal set of sensors for the Bun ecosystem:
    - HTTP sensor that can be plugged into the Bun HTTP stack we standardize on (for example, the Fizz HTTP server), capturing method, URL, status code, duration, and user context.
    - WebSocket sensor integrated into the Fizz Pusher server to track connection counts, channel subscriptions, and error rates.
...
- **First Step**: Build a `nightwatch-bun` package that only implements the TCP client and an HTTP-style sensor wired into the Fizz HTTP server. Once stable, layer in additional sensors for WebSockets and jobs.
...
- **Bun Strategy**:
  - Create `@fizz/stream-react` and `@fizz/stream-vue` packages that mirror the Laravel hook APIs but are backend-agnostic.
  ...
  - Ensure the hooks compose cleanly with Fizz-based apps, e.g. by using streams for unidirectional server output and WebSockets (via Sip) for bidirectional real-time updates.
- **Challenges**:
...
- **Bun Strategy**:
  - Instead of porting the Blade DSL directly, provide a Bun-native deployment / task runner (`fizz envoy`) driven by a concise TypeScript configuration file.
  ...
  - Design a small composable API so tasks can be defined in `envoy.config.ts` or programmatically in scripts, with presets tailored to Fizz (deploying the WebSocket server, running database migrations, restarting processes).
...
- **First Step**: Define an `envoy.config.ts` schema and implement a minimal CLI command (`fizz envoy <task>`) that executes one task against one or more hosts using `Bun.spawn` and `ssh`.

### 16. WebSockets Grid Demo (`websockets-grid-demo`)
- **Profile**: Real-time collaborative emoji grid demo built with Laravel 12, React 19, Inertia, and Laravel Fizz.
- **Core Concept**: A 10×10 interactive grid where clicking cells levels up emojis through several stages, backed by cache persistence and Fizz broadcasting. The demo showcases optimistic UI updates, `toOthers()` broadcasting, presence-based user counts, and celebratory "emoji rain" when the grid is filled.
- **Bun Strategy**:
  - Keep the existing Fizz chat demo app and add a second UI surface: a `Grid.tsx` page that is a near drop-in port of the Laravel `resources/js/pages/Grid.tsx` component.
  - Replace Laravel's HTTP backend with a small Bun HTTP API in the demo app that stores grid state (Redis or in-memory), tracks active users, and emits the same kinds of events (cell updates, rain events, user count changes) via Fizz channels.
  - Use `@fizz/sip/react` on the frontend to subscribe to the grid channels and apply updates, preserving the `toOthers()` behaviour by ensuring HTTP mutations include the current `X-Socket-Id` so originating clients do not receive duplicate updates.
...
- **Challenges**:
  ...
  - Mapping Laravel's broadcasting configuration and event naming to Fizz so that documentation and mental models transfer cleanly.
- **First Step**: Add a `Grid.tsx` React component to the demo frontend that hard-codes a 10×10 grid and wires up Sip subscriptions for cell updates and user count, backed by a minimal Bun HTTP handler that mutates grid state and triggers Fizz events.
