...
I want you to read .claude/commands/plan.md and create a spec to implement that main server factory. we should do so respecting the balance between the port source in fizz/ and this typescript/bun implementation`
task_type: feature
complexity: medium

## Task Description

Create a programmatic server factory entrypoint (`createServer`) that library consumers can import from `fizz` instead of shelling out to the CLI. The helper should wrap the existing `Factory` class, accept resolved configuration or a path to load, mirror Laravel Fizz’s expectations for host/port/path/TLS handling, and return a handle that exposes the Bun server plus cleanup helpers. The CLI must be refactored to consume this helper so there is a single code path for server bootstrapping.

## Objective

Expose a reusable `createServer` API that initializes the Fizz factory, builds a Bun server using the same semantics as Laravel’s PHP factory, wires optional diagnostics (event logging + periodic jobs), and surfaces lifecycle hooks so both the CLI and embedded users can boot/shutdown Fizz safely.

## Problem Statement

Currently, the only way to start Fizz is through `src/cli.ts`, which manually loads config, initializes the factory, spins up periodic jobs, and attaches signal handlers. There is no exported function, so embedding Fizz in another Bun/TypeScript project (or testing it programmatically) is awkward. The duplication also increases risk of diverging from Laravel’s upstream `Laravel\Fizz\Servers\Fizz\Factory` behavior because any future changes would need to be mirrored in multiple places.

## Solution Approach

Introduce a first-class `createServer` helper inside `src/servers/fizz/factory.ts` that wraps the existing `Factory` class. It should:

- Accept either a resolved `FizzConfig` object, a config path, or fall back to `loadConfig`, plus override flags (host, port, path, hostname, server name, TLS options, debug flag, automation toggles).
...
- `src/servers/fizz/factory.ts` – Home of the `Factory` class; house the new `createServer` helper, lifecycle utilities, and any shared types.
...
- `README.md` / `docs.md` – Document the new API so developers know how to start Fizz programmatically.
- `fizz/src/Servers/Fizz/Factory.php` – Reference implementation from upstream Laravel Fizz to ensure TLS, routing, and controller wiring stay in parity.
...
- Compare `src/servers/fizz/factory.ts` with `fizz/src/Servers/Fizz/Factory.php` to ensure the new helper does not miss any TLS/router responsibilities.
...
- Move `setupEventListeners`, `setupPeriodicTasks`, and `setupGracefulShutdown` into reusable functions (likely exported from `src/servers/fizz/factory.ts` or a new helper module) so they can be called both from the CLI and from `createServer` when requested.
...
- In `src/servers/fizz/factory.ts`, implement `createServer` that:
  - Resolves `FizzConfig` via direct argument, config path, or `loadConfig()` fallback.
...
- Add `export { createServer } from "./servers/fizz/factory";` to `src/index.ts`.
- Update README/docs with a small example of embedding Fizz via `createServer` (include TypeScript snippet showing config overrides and manual shutdown).
...
- `createServer` exists in `src/servers/fizz/factory.ts`, is exported from `src/index.ts`, and returns a structured result with cleanup helpers.
